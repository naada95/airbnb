...
export class OrderService {
 ...
 ORDER_URL = 'bookings/'
  ...
  public getCurrOrder() {
    ...
    // Si hôte : bookings/listing/:id, sinon : bookings/my
    let endpoint = filterBy.hostId
      ? `${this.ORDER_URL}listing/${filterBy.hostId}`
      : `${this.ORDER_URL}my`
    const orders = await this.httpService.get(endpoint, null)
  ...
  public query(filterBy: FilterOrder | null) {
    ...
    if (order._id) {
      ...
      return await this.httpService.patch(`${this.ORDER_URL}${order._id}/confirm`, order)  // ← patch
    }
    this.socketService.emit(this.socketService.SOCKET_EVENT_ORDER_FOR_HOST, order)
    return await this.httpService.post(this.ORDER_URL, order)
  }
