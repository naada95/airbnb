...
export class StayService {
  ...
  public async loadStays(index: number = 0, isConnectStays: boolean = true) {
    ...
    const stays = await this.httpService.get(this.STAY_URL + queryParams, null) as Stay[]
  ...
  public async query(filterBy: StayFilter): Promise<Stay[]> {
    ...
    return this.httpService.get(this.STAY_URL + queryParams, null) as Promise<Stay[]>
  }
  ...
  public async loadFullLength() {
    ...
    const stayLength = await this.httpService.get(this.STAY_URL + 'length/' + queryParams, null) as number
    this._stayLength$.next(stayLength)
  }

  public getById(stayId: string): Observable<Stay> {
    return this.httpService.get(this.STAY_URL + stayId, null) as unknown as Observable<Stay>
  }
